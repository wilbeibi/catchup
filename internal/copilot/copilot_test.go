...
// events mirrors a real Copilot CLI log (@github/copilot 1.0.80), trimmed to
// one event of each kind the provider must handle: the lifecycle rows, the
// system prompt, a tool-only assistant turn, the tool plumbing around it, and
// a sub-agent's messages — all of which are noise — plus the two human turns
// and the two answers that are not.
...
{"type":"subagent.user.message","id":"e11","timestamp":"2026-08-24T15:07:51.000Z","data":{"content":"sub-agent instructions"}}
{"type":"subagent.assistant.message","id":"e12","timestamp":"2026-08-24T15:07:51.500Z","data":{"model":"gpt-5-mini","content":"sub-agent answer"}}
{"type":"assistant.turn_end","id":"e13","timestamp":"2026-08-24T15:07:52.000Z","data":{"turnId":"0"}}
{"type":"session.compaction_complete","id":"e14","timestamp":"2026-08-24T15:08:00.000Z","data":{"success":true,"compactionTokensUsed":1234}}
...
const workspace = `id: 816a9dd1-c2ef-4b36-99a8-b503320856cc
...
func wantEntries(t *testing.T) []session.Entry {
	return []session.Entry{
		...
		{Kind: session.KindCompact, Time: ts(t, "2026-08-24T15:08:00.000Z")},
...
func TestReadSkipsEverythingButConversation(t *testing.T) {
	root := t.TempDir()
	writeSession(t, root, "816a9dd1", workspace, events, time.Now())
	...
	// The id comes from workspace.yaml, which spells it in full; the
	// directory name is only the fallback.
	if src.Ref.SessionID != "816a9dd1-c2ef-4b36-99a8-b503320856cc" {
...
func TestListOrdersByEventLogMtime(t *testing.T) {
	...
	writeSession(t, root, "newer", strings.Replace(workspace, "816a9dd1-c2ef-4b36-99a8-b503320856cc", "newer-id", 1), events, now)
